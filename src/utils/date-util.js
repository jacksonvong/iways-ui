...
let fecha = Vue.prototype.$isServer ? function() {} : require('./date')
...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`iw.datepicker.weeks.${week}`)),
    dayNames: weeks.map(week => t(`iw.datepicker.weeks.${week}`)),
    monthNamesShort: months.map(month => t(`iw.datepicker.months.${month}`)),
    monthNames: months.map((month, index) => t(`iw.datepicker.month${index + 1}`)),
    amPm: ['am', 'pm']
  }
}

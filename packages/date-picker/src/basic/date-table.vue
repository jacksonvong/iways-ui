<template>
  <table
    ...
    <tbody>
      <tr>
        <th v-if="showWeekNumber">{{ t('iw.datepicker.week') }}</th>
        <th v-for="(week, key) in WEEKS" :key="key">{{ t('iw.datepicker.weeks.' + week) }}</th>
      </tr>
      <tr
        v-for="(row, key) in rows"
        :class="{ current: isWeekActive(row[1]) }"
        :key="key"
        class="iw-date-table__row">
        <td
...
<script>
import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, getStartDateOfMonth, prevDate, nextDate, isDate, clearTime as _clearTime } from '@iways-ui/src/utils/date-util'
import Locale from '@iways-ui/src/mixins/locale'
import { arrayFindIndex, arrayFind, coerceTruthyValueToArray } from '@iways-ui/src/utils/util'
...
export default {
    ...
    rows() {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
          ...
          if (this.selectionSide === 'left' && this.maxDate && cellDate > this.maxDate) {
            cell.disabled = true
          }
          if (this.selectionSide === 'right' && this.minDate && cellDate < this.minDate) {
            cell.disabled = true
          }
  ...
  methods: {
    ...
    markRange(minDate, maxDate, endDate) {
      minDate = minDate ? getDateTimestamp(minDate) : minDate
      maxDate = maxDate ? getDateTimestamp(maxDate) : maxDate
      endDate = endDate ? getDateTimestamp(endDate) : endDate

<template>
  <transition name="iw-zoom-in-top" @after-enter="handleEnter" @after-leave="handleLeave">
    <div
      v-show="visible"
      :class="[{
      ...
      class="iw-picker-panel iw-date-picker iw-popper">
      <div class="iw-picker-panel__body-wrapper">
        ...
        <div class="iw-picker-panel__body">
          <div v-if="showTime" class="iw-date-picker__time-header">
            <span class="iw-date-picker__editor-wrap">
              <iw-input
                :placeholder="t('iw.datepicker.selectDate')"
                :value="visibleDate"
                size="small"
                @input="val => userInputDate = val"
                @change="handleVisibleDateChange" />
            </span>
            <span v-clickoutside="handleTimePickClose" class="iw-date-picker__editor-wrap">
              <iw-input
                ref="input"
                :placeholder="t('iw.datepicker.selectTime')"
                :value="visibleTime"
                size="small"
          ...
          <div
            ...
            :class="{ 'iw-date-picker__header--bordered': currentView === 'year' || currentView === 'month' }"
            class="iw-date-picker__header">
            <button
              :aria-label="t(`iw.datepicker.prevYear`)"
              type="button"
              class="iw-picker-panel__icon-btn iw-date-picker__prev-btn iw-icon-d-arrow-left"
              @click="prevYear"/>
            <button
              v-show="currentView === 'date'"
              :aria-label="t(`iw.datepicker.prevMonth`)"
              type="button"
              class="iw-picker-panel__icon-btn iw-date-picker__prev-btn iw-icon-arrow-left"
              @click="prevMonth"/>
            <span
              role="button"
              class="iw-date-picker__header-label"
            ...
            <span
              ...
              role="button"
              class="iw-date-picker__header-label"
              @click="showMonthPicker">{{ t(`iw.datepicker.month${ month + 1 }`) }}</span>
            <button
              :aria-label="t(`iw.datepicker.nextYear`)"
              type="button"
              class="iw-picker-panel__icon-btn iw-date-picker__next-btn iw-icon-d-arrow-right"
              @click="nextYear"/>
            <button
              v-show="currentView === 'date'"
              :aria-label="t(`iw.datepicker.nextMonth`)"
              type="button"
              class="iw-picker-panel__icon-btn iw-date-picker__next-btn iw-icon-arrow-right"
              @click="nextMonth"/>
          </div>

          <div class="iw-picker-panel__content">
            <date-table
              v-show="currentView === 'date'"
...
import IwInput from '@iways-ui/packages/input'
import IwButton from '@iways-ui/packages/button'
...
export default {
  ...
  components: {
    TimePicker, YearTable, MonthTable, QuarterTable, DateTable, IwInput, IwButton
  },
  mixins: [Locale],
  ...
  computed: {
    ...
    yearLabel() {
      const yearTranslation = this.t('iw.datepicker.year')

<template>
  <transition name="iw-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      ref="popper"
      :style="{ width: width + 'px' }"
      :class="popperClass"
      class="iw-picker-panel time-select iw-popper">
      <iw-scrollbar noresize wrap-class="iw-picker-panel__content">
        <div
          v-for="item in items"
...
import scrollIntoView from '@iways-ui/src/utils/scroll-into-view'
...
export default {
  ...
  methods: {
    ...
    scrollToOption(selector = '.selected') {
      const menu = this.$refs.popper.querySelector('.iw-picker-panel__content')
